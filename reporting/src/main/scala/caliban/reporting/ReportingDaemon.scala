...
object ReportingDaemon {
  ...
  def register(ref: SchemaReportingRef[_]): URManaged[ReportingDaemon, Unit] =
    ZManaged.serviceWithManaged(_.register(ref))

  def live: ZLayer[Clock with SchemaReporter, Nothing, ReportingDaemon] =
    make.toLayer

  def make: ZManaged[Clock with SchemaReporter, Nothing, ReportingDaemon] =
    ZManaged.suspend {
      for {
