...
final private class ObjectSchema[R, A](
  ...
  // Scala Native does not include java.lang.reflect.Method in its javalib, so method-based
  // field resolution (zero-arg methods on case classes) is unsupported on this platform.
  // Constructor-derived fields work as on the JVM.
  @threadUnsafe
          ...
          methodName =>
            (_: A) =>
              throw new UnsupportedOperationException(
                s"Method-based GraphQL field '$methodName' is not supported on Scala Native"
              )
        )
    }
