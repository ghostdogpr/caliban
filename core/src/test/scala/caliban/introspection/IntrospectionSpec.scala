  ...
  override def spec =
    suite("IntrospectionSpec")(
      ...
      test("introspect type") {
        ...
        interpreter.flatMap(_.execute(query)).map(_.data.toString).map { response =>
      ...
      test("introspect nonexistent type") {
        ...
        interpreter.flatMap(_.execute(query)).map(_.data.toString).map { response =>
          assertTrue(response == """{"__type":null}""")
        }
      },
      test("introspect the introspection itself") {
        val interpreter = graphQL(resolverIO).interpreter
        val query       = gqldoc("""
              query IntrospectionQuery {
