...
object Annotations {
   ...
   */
  final case class GQLDeprecated(reason: String) extends StaticAnnotation

  /**
   * Annotation used to provide a description to a field or a type.
   */
  final case class GQLDescription(value: String) extends StaticAnnotation
   ...
   */
  final case class GQLInputName(name: String) extends StaticAnnotation

  /**
   * Annotation used to provide an alternative name to a field or a type.
   */
  final case class GQLName(value: String) extends StaticAnnotation

  /**
   * Annotation used to provide directives to a schema type
   */
  final case class GQLDirective(directive: Directive) extends StaticAnnotation

  /**
   * Annotation to make a sealed trait an interface instead of a union type or an enum
   */
  final case class GQLInterface() extends StaticAnnotation

  /**
   * Annotation to make a sealed trait a union instead of an enum
   */
  final case class GQLUnion() extends StaticAnnotation

  /**
   * Annotation to make a union or interface redirect to a value type
   */
  final case class GQLValueType() extends StaticAnnotation

  /**
   * Annotation to specify the default value of an input field
   */
  final case class GQLDefault(value: String) extends StaticAnnotation
}
