...
trait GraphQL[-R] { self =>
  ...
  private lazy val cachedInterpreter: Either[ValidationError, GraphQLInterpreter[R, CalibanError]] =
            ...
            case (overallWrappers, parsingWrappers, validationWrappers, executionWrappers, fieldWrappers, _) =>
              wrap((request: GraphQLRequest) =>
                (for {
                  ...
                  result                               <- wrap(resolve(op, fieldWrappers, intro))(executionWrappers, executionRequest)
