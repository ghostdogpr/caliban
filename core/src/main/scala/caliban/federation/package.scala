...
package object federation {
  ...
  private val _FieldSet = __InputValue(
    "fields",
    None,
  ...
  private val federationDirectives = List(
    __Directive(
      "external",
  ...
  def federate[R](underlying: GraphQL[R], resolvers: List[EntityResolver[R]]): GraphQL[R] = {
    ...
    implicit val anySchema: Schema[Any, _Any] =
      scalarSchema("_Any", None, _ => NullValue)
    ...
    implicit val entitySchema: Schema[R, _Entity] = new Schema[R, _Entity] {
      ...
      private lazy val _entityMap = resolvers.flatMap { r =>
        r.toType.name.map(_ -> r)
      }.toMap
    ...
    GraphQL.graphQL[R, Query, Unit, Unit](
      RootResolver(
        Query(
          _entities = args =>
            args.representations.map { rep =>
              _Entity(rep.__typename, rep.fields)
          },
