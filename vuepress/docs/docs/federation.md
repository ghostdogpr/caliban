...
If you already have a graph you can add federation simply by calling the wrapper function
...
val federatedSchema: GraphQL[R] = federated(schema)
```
...
```
federated(schema, aResolver, additionalResolvers:_*)
```

You can now use the resulting `GraphQL[R]` to start querying.
