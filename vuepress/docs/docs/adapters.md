...
```scala
libraryDependencies += "com.github.ghostdogpr" %% "caliban-quick" % "3.1.5"
```
...
- `withMaxRequestBodyBytes` which changes the 1 MB default limit for JSON and GraphQL request bodies.
- `withMaxUploadBodyBytes` which changes the 16 MB default limit for multipart upload bodies.
- `withMaxResponseBodyBytes` which changes the 16 MB default limit for JSON response bodies.
