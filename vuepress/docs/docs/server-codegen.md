...
- `missingInputEffect`: An effect for a missed input field in a form of `F[*]` (e.g. `scala.Either[Unit, *]`). Specifying it will make all fields of input types for mutations optional.
This allows to distinguish the cases when the field is not passed and when the field is passed with `null` value (`None` in scala).
