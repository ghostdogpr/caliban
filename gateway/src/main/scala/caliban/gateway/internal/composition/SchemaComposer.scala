...
private[composition] final class SchemaComposer private (subgraphs: List[PreparedSubgraph]) {
  ...
  private val namesBySubgraph    = sortedSubgraphs.map(subgraph => subgraph -> federationDirectiveNames(subgraph.document))
  ...
  private def subgraphType(
    ...
    SubgraphType(
      ...
      inaccessibleInputFields = subgraph.mapping.hiddenInputFields.collect { case TypeField(`name`, field) => field } ++
  ...
  private def compileSecurity(metadata: SubgraphMetadata): Either[List[String], List[SecurityDirectiveApplication]] = {
  ...
  private def interfaceOverrideDiagnostics: List[String] = {
    ...
    def owns(source: String, interfaceName: String, field: String): Boolean =
      types.exists(entry => entry.source == source && entry.name == interfaceName && entry.ownedFields.contains(field))
...
private[gateway] object SchemaComposer {
  ...
  def isFederation(document: Document): Boolean =
    isFederation2(document) || {
      ...
      typeNames.contains(AnyType) && typeNames.contains("_Entity") &&
      document.objectTypeDefinitions.exists(_.fields.exists(_.name == EntitiesField))
    }
  ...
  private def isFederation2(document: Document): Boolean = {
    val schemaExtensionDirectives =
      document.typeExtensions.collect { case extension: SchemaExtension => extension }.flatMap(_.directives)
    (linkedFeatures(document) ::: linkedFeatures(schemaExtensionDirectives)).exists(_.identity == FederationIdentity)
  }
