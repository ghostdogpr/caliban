...
private[composition] final class ContextCompilation(typesBySource: Map[SourceType, SubgraphType]) {
  ...
  def compile(
    ...
    if (!subgraph.federation || !names.supportsContexts) Right(FederationContexts(Nil, Nil))
      ...
      val typesByContext    = contextDirectives.flatMap { case (typeName, directive) =>
        stringArgument(directive.arguments, "name").map(name => ContextName(name) -> typeName)
      }.groupMap(_._1)(_._2)
  ...
  private def validateContextReceiver(source: String, typeName: String, fieldName: String): Either[String, Unit] = {
    val receiver        = typesBySource.get(SourceType(source, typeName))
    val parent          = receiver.map(_.tpe)
    val implementsField =
      parent.toList.flatMap(_.interfaces().getOrElse(Nil)).exists(fieldDefinition(_, fieldName).nonEmpty)
    val isEntityObject  =
  ...
  private def isInterfaceObject(source: String, typeName: String): Boolean =
    typesBySource.get(SourceType(source, typeName)).exists(_.interfaceObject)
  ...
  private def contextSelectionTypes(
    ...
    def applies(fragment: Selection.InlineFragment, runtime: __Type): Boolean =
      fragment.typeCondition.forall(condition =>
        rootType.types.get(condition.name).exists(contextPossibleTypes(_).exists(_.name == runtime.name))
      )
    ...
    for {
      ...
      values <- traverseEither(contextPossibleTypes(parent))(resolve(parent, _, selections))
...
private[composition] object ContextCompilation {
  ...
  private def validateContextSelectionSyntax(selections: List[Selection]): Either[String, Unit] =
    traverseEither(selections) {
      ...
      case Selection.InlineFragment(_, directives, children)     =>
  ...
  private def validateContextTypeConditions(
    ...
    def matchesNoLocation(condition: String): Boolean =
      rootType.types
        .get(condition)
        .forall(tpe => contextPossibleTypes(tpe).flatMap(_.name).forall(!locationTypes(_)))
