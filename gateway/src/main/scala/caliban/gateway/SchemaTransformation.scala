 ...
 * `caliban.transformers.Transformer.ExcludeArgument`. Gateway transformations also rewrite downstream
 * operations and responses, so hidden or renamed subgraph coordinates stay routable.
 */
