...
trait CalibanKeys {
  ...
  lazy val calibanGenerator: TaskKey[Seq[File]] = taskKey[Seq[File]]("Generate GraphQL sources using caliban-codegen-sbt")

  lazy val calibanSources: SettingKey[File]  = settingKey[File]("Where to find .graphql schemas")
  lazy val calibanSettings: SettingKey[Seq[CalibanSettings]] = settingKey[Seq[CalibanSettings]]("Settings that apply to individual GraphQL files")
