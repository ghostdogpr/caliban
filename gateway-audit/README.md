...
The upstream repository and commit live in [`upstream.env`](upstream.env). CI runs the unmodified upstream reporter so its `suite_index` case identities and raw assertion results remain intact. [`supported-cases.txt`](supported-cases.txt) protects every passing identity from regression, while [`expectations.tsv`](expectations.tsv) assigns ownership to known failures. Together they classify cases as:
...
`verify-results.sh` combines the raw upstream result with the passing baseline and disposition table into a case-level Markdown report. It rejects regressions, missing cases, and newly unclassified suites; it never treats an assertion failure as flaky.
...
"$CALIBAN_ROOT/gateway-audit/verify-results.sh" \
  ...
  "$CALIBAN_ROOT/gateway-audit/supported-cases.txt" \
