...
object TapirSttpClientZioWebSockets {
  ...
  class WebSocketToZioPipe[R <: ZioStreams with WebSockets] extends WebSocketToPipe[R] {
    ...
    override def apply[REQ, RESP](
      s: Any
    )(ws: WebSocket[F], o: WebSocketBodyOutput[Any, REQ, RESP, _, ZioStreams]): Any = { (in: Stream[Throwable, REQ]) =>
      val sends: ZStream[Any, Throwable, Unit] = in
        .map(o.requests.encode)
        .mapZIO(ws.send(_, isContinuation = false))
